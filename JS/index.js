...
async function getData(land) {
    try {
        ...
        const row = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=107967fb950640699cf222544240712&q=${land}&days=3&aqi=yes&alerts=yes`);
...
async function displayData(land) {
    const weatherData = await getData(land);
    ...
    if (weatherData && weatherData.forecast) {
        ...
        cardData.innerHTML = `<p class="text-white mx-auto text-center fs-5">Unable
...
// Automatically display the weather for "Giza" when the page loads
